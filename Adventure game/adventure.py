...
def check_win_state(player):
    if player.place != airport:
        return False
    print()
    backpack = player.backpack
    if ticket in backpack:
        return True
    else:
        print()
        print("Looks like you're missing a ticket. You cannot board a flight to the USA")
        return False
...
You are Abdul. An Afghan-American translater serving in the US marines. 
You are deep in rural Afghanistan during the fall of Kabul.
You must find a way to get to the Kabul International Airport, and catch a flight USA without getting cuaght.

""".format(me.name if isinstance(me, Player) else '______')

WIN_MESSAGE = """
Congratulations! You boarded a flight and made it safely to the United States. 
Thank you for your service as a translator!
Game Over.
"""
